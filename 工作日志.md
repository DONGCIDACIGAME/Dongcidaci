...
5. 音频节奏提取，部分参考资料
    ...
    4. https://dida.do/blog/beat-tracking-with-deep-neural-networks
...
# 2023/07/21
1. 计划新增一个攻击状态，不等节拍，按原速度播放，播放完完成后直接切换
...
# 2023/07/31
1. 是否可以在攻击间加一个状态，有小幅度移动，衔接攻击，如果加了，该怎么和移动状态进行切换？节拍处换run？
2. 攻击间如果会切到idle导致combo失败会有点难受

# 2023/08/01
1. 转身速率是否可以和转向幅度有正相关性？
2. transition之后加一个小跑状态，移动速度降低看看效果

# 2023/08/06
1. 【DONE】agentmgr，能否不做成单例，改为一个实例对象，mapmgr？
2. 【DONE】场景中固定的节点，目前获取时分散在不同的类里，能否放到一个指定的单例中，所有取场景节点的地方，通过这个单例获取，单例去获取场景中的节点，是分散到各个场景中的。这样一些子实例就不再依赖上层控制类中的节点了

# 2023/08/07
1. 【DONE】bpm高的音乐，一些攻击会被吃掉，查下原因  
    - 过度动画跨拍可能会导致过拍后的transition部分的指令，因为transition只在节拍处才执行攻击的指令逻辑，但是这种情况下，transition已经过了节拍，接下来新的指令会顶掉原来的指令
    - 指令进了缓存，进transition之前要再peek一下
2. ~~相机跟随，不要完全跟随~~ `相机跟随感觉比不跟随要好一点，整体画面更跟手，视觉中心更舒服，但是考虑下角色是否会有震动的场景，如果有的话需要划分一下角色的root节点和相机跟随节点，震动只作用于角色root`
3. ~~感觉手感上还是有一些拖沓~~ `攻击动画时间内的攻击指令，进入指令缓存区，在攻击动作结束后直接进新的combo，不再进过渡态的。在过渡态内的攻击指令，打断过渡态直接进新的combo`

# 2023/08/10
1. 【ONGOING】计划开始制作蓄力攻击状态
2. 【DONE】冲刺攻击的即时响应也加一下

# 2023/08/14
1. 动画驱动器 现在只有步进式驱动器支持配置文件里的动画播放次数，默认驱动器和卡节拍驱动器都默认只播放一次动画，这里需要都支持根据配置的播放次数播放动画，但是涉及到一个问题，现在执行一个指令后，逻辑结束拍是根据动画配置文件里的配置算出来的，如果是根据播放次数算出逻辑结束拍，中间每次播完都忽略的话，会不会导致一些逻辑没办法处理？`应该没有问题，配置上如果是播两次，那动画应该就是播两次的，动画管动画播放，逻辑上不依赖动画播没播完，逻辑上只计算逻辑完成拍，这个逻辑完成拍也是根据动画时长和执行次数决定的，理论上应该能和动画匹配上`
2. 【DONE】动画驱动器准备改为自驱动（步进式动画驱动目前还是状态驱动的），外部传入参数后动画播放完全交由驱动器驱动了，状态里只计算一个逻辑结束拍  `配置多次循环的测试下`
3. 【DONE】蓄力攻击可能为步进式驱动，支持顺序播放后循环，卡节拍，但是会截断，不知道会不会奇怪，测试下 `卡拍动画驱动`
4. 【DONE】动画的位移控制，目前没有做循环播放时的逻辑，需要补充


# 2023/08/16
1. 【DONE】蓄力攻击现在很奇怪
2. 状态的默认效果没有执行逻辑，需要处理下

# 2023/08/20
1. 蓄力攻击的拍子时长要怎么处理？现在松手即打出蓄力攻击，卡拍子的，会导致被截断，可能两拍的旋转变成了一拍的，目前改为如果抬手时，在前半拍，按照原来的拍子循环次数播放，如果是在后半拍，则多给一拍 ~~`目前逻辑上改为了等拍子，然后走完拍子时长，但是会卡手，需要优化`~~


# 2023/08/22
1. ComboEffectExcutor 需要进行改造，后续应该不仅仅是给combo用的，蓄力攻击也要用这个效果执行器，需要支持动画循环时，效果也能匹配循环触发

# 2023/08/30
1. EffectExcutor有个很大的问题，由于可能存在动画被截断，所以应该再执行效果时，跳过被截断那一段的效果，效果结算时间也应该从动画截断处开始计时，现在这块逻辑都是从头开始的，逻辑上，这个执行逻辑和使用哪种动画播放方式也有直接关系，这块可能需要大的重构
2. 有一个
