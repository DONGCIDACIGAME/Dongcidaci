...
# 2023/05/24
1. 所有计算结束拍的逻辑进行了调整，比如第一拍开始，占1拍，那结束拍还是第一拍，即第一拍结尾处结束
2. 传指令的地方，统一改为传指令数据（类型，方向，节拍）
3. 新增了状态的默认逻辑代码段，计划把combo改回覆盖和叠加模式
4. status配置，删除loop字段和animName字段

# 2023/05/25
1. OnNormalCommand和OnComboCommand进行合并
2. 添加状态的默认行为逻辑
